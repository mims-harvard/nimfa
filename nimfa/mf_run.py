...
"""
    ...
    This module implements the main interface to launch matrix factorization
    algorithms. MF algorithms can be combined with implemented seeding methods.
    
    Returned object can be directly passed to visualization or comparison
    utilities or as initialization to another factorization method.
    
    #. [mandatory] Choose the MF model by specifying the algorithm to perform
    MF on target matrix.
    #. Choose the number of runs of the MF algorithm. Useful for achieving
    stability when using random seeding method.
    #. Pass a callback function which is called after each run when performing
    multiple runs of the algorithm. Useful for saving summary measures or
    processing the result of each NMF fit before it gets discarded. The callback
    function is called after each run.
    #. [mandatory] Choose the factorization rank to achieve.
    #. [mandatory] Choose the seeding method to compute the starting point
...
def mf(target, seed=None, W=None, H=None,
       ...
       n_run=1, callback=None, callback_init=None, initialize_only=True,
       **options):
    """
    Run the specified MF algorithm.
    
    Return fitted factorization model storing MF results. If
    :param:`initialize_only` is set, only initialized model is returned
    (default behaviour).
    
    :param target: The target matrix to estimate. Some algorithms (e. g.
    multiple NMF) specify more than one target matrix. In that case target
    matrices are passed as tuples. Internally, additional attributes with names
    following Vn pattern are created, where n is the consecutive index of target
    matrix. Zero index is omitted (there are V, V1, V2, V3, etc. matrices and
    then H, H1, H2, etc. and W, W1, W2, etc. respectively).
    :type target: Instance of the :class:`scipy.sparse` sparse matrices types,
    ...
    :param H: Specify initial factorization of mixture matrix H. In case of
    factorizations with multiple MF underlying model, initialization of multiple
    mixture matrices can be passed as tuples (in order H, H1, H2, etc.
    respectively). Default is None. When specified, :param:`seed` must be None.
    ...
    :param method: The algorithm to use to perform MF on target matrix. Default
    is :class:`methods.factorization.nmf.Nmf`
    :type method: `str` naming the algorithm or :
    class:`methods.factorization.bd.Bd`, :class:`methods.factorization.icm.Icm`,
    :class:`methods.factorization.Lfnmf.Lfnmf`,
    :class:`methods.factorization.lsnmf.Lsnmf`,
    :class:`methods.factorization.nmf.Nmf`,
    :class:`methods.factorization.nsnmf.Nsmf`,
    :class:`methods.factorization.pmf.Pmf`,
    :class:`methods.factorization.psmf.Psmf`,
    :class:`methods.factorization.snmf.Snmf`,
    :class:`methods.factorization.bmf.Bmf`,
    :class:`methods.factorization.snmnmf.Snmnmf`
    ...
    :param initialize_only: The specified MF model and its parameters will only
    be initialized. Model initialization includes:
        #. target matrix format checking and possibly conversion into one of
        accepting formats,
        #. checking if target matrix (or matrices) are nonnegative (in case of
        NMF factorization algorithms),
        #. validation of the specified factorization method,
        #. validation of the specified initialization method.
    When this parameter is specified factorization will not be ran. Default
    is True.
    ...
    **Runtime specific parameters**
    ...
    In addition to general parameters above, there is a possibility to specify
    runtime specific and factorization algorithm specific options.
    
    .. note:: For details on algorithm specific options see specific algorithm
    documentation.
    ...
    **Stopping criteria parameters**
    ...
    If multiple criteria are passed, the satisfiability of one terminates the
    factorization run.
    
    .. note:: Some factorization and initialization methods have beside the
    following also algorithm specific stopping criteria. For these details see
    specific algorithm's documentation.
    ...
    try:
        if isinstance(method, str):
            mf_model = methods.factorization.methods[method.lower(
               ...
               rank=rank, max_iter=max_iter, min_residuals=min_residuals,
               test_conv=test_conv, n_run=n_run, callback=callback,
               callback_init=callback_init, options=options)
        else:
            mf_model = method(
                V=target, seed=seed, W=W, H=H, H1=None, rank=rank,
                max_iter=max_iter, min_residuals=min_residuals,
                test_conv=test_conv, n_run=n_run, callback=callback,
                callback_init=callback_init, options=options)
